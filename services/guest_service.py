...
class GuestService:
    ...
    def create_guest(phone: str, first_name: str, last_name: Optional[str] = None):
        return (
            supabase_admin_client.table("guests")
            .insert({"phone": phone, "first_name": first_name, "last_name": last_name})
            .execute()
        )
    ...
    def get_or_create_guest(
        phone: str, first_name: str, last_name: Optional[str] = None
    ):
        # Normalize the phone number
        if not phone.startswith("+1"):
            phone = "+1" + phone.lstrip(
                "1"
    ...
    @staticmethod
    def update_guest(
        guest_id: int, name: Optional[str] = None, email: Optional[str] = None
    ):
        update_data = {}
        if name:
    ...
    def delete_guest(guest_id: int):
        return (
            supabase_admin_client.table("guests").delete().eq("id", guest_id).execute()
        )
