...
class AmastayScraper:
    def __init__(self, base_url: str, api_key: str):
        self.base_url = base_url
        self.api_key = api_key

    def scrape_property_data(self, property_url: str) -> dict:
        """Scrape property data from the given URL."""
        headers = {
            "Authorization": f"Bearer {self.api_key}",
        }
        response = requests.get(f"{self.base_url}/scrape", headers=headers, params={"url": property_url})

        if response.status_code != 200:
            logging.error(f"Scraping failed with status: {response.status_code}")
            raise ValueError(f"Scraping failed with status: {response.status_code}")
